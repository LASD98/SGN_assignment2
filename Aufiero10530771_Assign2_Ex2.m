...
% Assignment #2 - Guidance - Exercise 2 Batch filters
% Author: Luca Aufiero
% Personal code: 10530771
% Matricola MSc Degree: 232972
% Total expected execution time ~ 100 s
%
...
typerun    = 'u';  % user-provided inputs to SGP4 Matlab functions
...
trPorr1 = 3 * sqrt(max(eig(P_ls1(1:3,1:3))));
trPovv1 = 3 * sqrt(max(eig(P_ls1(4:6,4:6))));

Jac2 = full(jacobian_12);
P_ls2 = resnorm_12/(length(residual_12)-length(x0)).*inv(Jac2.'*Jac2);
trPorr12 = 3 * sqrt(max(eig(P_ls2(1:3,1:3))));
trPovv12 = 3 * sqrt(max(eig(P_ls2(4:6,4:6))));

Jac_12_j2 = full(jacobian_12_j2);
P_ls_12_j2 = resnorm_12_j2/(length(residual_12_j2)-length(x0)).*inv(Jac_12_j2.'*Jac_12_j2);
trPorr12j2 = 3 * sqrt(max(eig(P_ls_12_j2(1:3,1:3))));
trPovv12j2 = 3 * sqrt(max(eig(P_ls_12_j2(4:6,4:6))));
...
Jac_sat2 = full(jacobian_sat2);
P_sat2 = resnorm_sat2/(length(residual_sat2)-length(x0_guess2)).*inv(Jac_sat2.'*Jac_sat2);
trPorrsat2 = 3 * sqrt(max(eig(P_sat2(1:3,1:3))));
trPovvsat2 = 3 * sqrt(max(eig(P_sat2(4:6,4:6))));
...
hold on
yline(Station1.min_el,'--r')
title(['@',Station1.name])
xlabel('Epoch [MJD2000]')
ylabel('Elevation [deg]')
legend(Sat1.name, 'Minimum elevation')

subplot(1,2,2)
plot(et/cspice_spd(), Station2.sat1.elevation*cspice_dpr(),'DisplayName', Sat1.name)
hold on
yline(Station2.min_el,'--r')
title(['@',Station2.name])
xlabel('Epoch [MJD2000]')
ylabel('Elevation [deg]')
legend(Sat1.name, 'Minimum elevation')
...
title(['@',Station1.name])
xlabel('Epoch [MJD2000]')
ylabel('Elevation with noise [deg]')
legend(Sat1.name, 'Minimum elevation')
...
% Measurements skyplots w/o noise
figure()
...
% Measurements skyplots with noise
figure()
...
title(['@',Station1.name, '(with noise)'])
...
title(['@',Station2.name, '(with noise)'])

% Norms of residuals to verify best combination
figure()
...
title('Mango norms of residuals');
xlabel('Measurements');
ylabel('[-]');

% Tango norm of residuals with best combination
figure()
semilogy(vecnorm(residual_sat2'))
legend('Kourou + Svalbard + J2')
xlabel('Measurements');
ylabel('[-]');
title('Tango norm of residuals')
...
%
%
%   INPUT:  stationName: name of the ground station
...
%               stationname: name of the ground station
%               stationname: name of the satellite
